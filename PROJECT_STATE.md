...
7. **✅ Live data fixed 2026-07-26** — root cause + fix in `01_DECISION_ANALYSIS.md` amendment 2026-07-26 (live-data audit + Yahoo fallback). Operator obtained a free `FRED_API_KEY` (in `.env`, gitignored) → 15/22 series live direct from FRED. Added `ipos/etl/yahoo.py` (free public chart endpoint) as a second leg for the 7 Stooq-only series with zero prior fallback (`SPX`, `NDX`, `RUT`, `DAX`, `GOLD`, `COPPER`, `EURUSD`, `US10Y_STOOQ`) → **first-ever real (non-synthetic) snapshot produced, `data/exports/snapshots/2026-07-24/`, status OK, 22/22 real.**
   ...
   **A THIRD instance of the same bug class was found 2026-07-27 — latent, not shipped.** `HY_OAS`/`IG_OAS` hold 64 weeks of synthetic canonical values (2022-05 → 2023-07) still in `fact_weekly`, with derived `fact_score` rows that carry no vintage and so look real. Today's scores are clean by a 7-day margin and the new `ipos-replay` refuses tainted weeks by default, but **those rows should be purged** before any deeper backfill or longer lookback — the only open action from that finding. Detail: `01_DECISION_ANALYSIS.md` amendment 2026-07-27 (later).
8. **UX/explainability improvement plan (2026-07-26), extended by the 2026-07-27 report review:**
   ...
   4. ⬜ General polish pass — remaining: an aggregate portfolio-level read, weight-history sparkline, and Δ-column sort/ordering.
