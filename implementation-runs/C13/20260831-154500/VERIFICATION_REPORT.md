...
| **2. Removing/denying Riskfolio causes explicit failure** | `test_m13_t10_anti_facade_riskfolio_denial` patches `rp.Portfolio.optimization` to simulate missing dependency; wrapper raises `OptimizationException("Riskfolio solver failed...")`. | Monkeypatch failure interception; AST check `test_m13_t09_anti_facade_no_scipy_in_module` proves zero `scipy.optimize` code exists. | **PASS** |
...
| **8. Offline execution / zero network egress** | Pure local memory array passing; no socket calls; `is_network_disabled=True` in diagnostics. | Offline execution in local `.venv`. | **PASS** |
